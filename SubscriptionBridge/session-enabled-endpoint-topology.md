...
found       -> receive batch, abandon the backlog prefix (hold-back),
               process ONLY the match, clear block on success
after clear -> the abandoned backlog re-flows in original order
```
...
- **Scheduled-resend delayed retries preserve order under a concurrent producer.** With `Customer-123`, `msg2` failed and was scheduled for a +6s resend while a concurrent publisher kept feeding `msg5/6/7` into the same session. Completion order came out `msg1 -> msg2 -> msg3 -> msg5 -> msg6 -> msg7`. The hold-back abandoned the backlog prefix repeatedly until the scheduled `msg2` arrived. That is the falsifiable test, and it held.
...
- **`MaxDeliveryCount` semantics.** Today the pump DLQs on first failure with no abandon retries, so the broker's `MaxDeliveryCount` is irrelevant. Once immediate retries via abandon exist, it silently becomes the retry budget unless the transport owns attempt tracking via `RetryCount`.
